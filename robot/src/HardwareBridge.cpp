...
/*!
 * LCM Handler for gamepad messageinitHardware();
 */
...
void MiniCheetahHardwareBridge::run() {
  ...
  if(_load_parameters_from_file) {
    ...
    while (!_robotParams.isFullyInitialized()) {
      // std:: cout << "_robotParams" << _robotParams << std::endl;
      printf("[bbb Hardware Bridge] Waiting for robot parameters...\n");
      usleep(1000000);
    }
...
void MiniCheetahHardwareBridge::runMicrostrain() {
  imu_receiver.start();
  while(true) {
    //_microstrainImu.run(); //original
    //imu_receiver.receiveData();
  //printf("imu_data = %.3f\n",imu_receiver.imu_data[1]);
...
void MiniCheetahHardwareBridge::initHardware() {
  ...
  // 初始化spi接口
  init_spi();
  // _microstrainInit = _microstrainImu.tryInit(0, 921600); //original
  _microstrainInit = imu_receiver.openPort("/dev/ttyUSB0", 921600); //new added
  printf("Test openPort bbb!!!\n");
}
