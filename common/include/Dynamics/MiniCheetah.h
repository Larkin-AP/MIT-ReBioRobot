...
//   //cheetah._jointDryFriction = .0;
  ...
  cheetah._motorTauMax = 4.f;
  cheetah._batteryV = 24;
  cheetah._motorKT = .3;  // this is flux linkage * pole pairs // 这个地方影响很大，要调参
  cheetah._motorR = 0.173;
  cheetah._jointDamping = .01;
  cheetah._jointDryFriction = .2;
  //cheetah._jointDamping = .0;
...
//   cheetah._motorTauMax = 4.f;
//   cheetah._batteryV = 24;
//   cheetah._motorKT = .87;  // this is flux linkage * pole pairs
//   cheetah._motorR = 0.173;
//   cheetah._jointDamping = .01;
//   cheetah._jointDryFriction = .2;
//   //cheetah._jointDamping = .0;
...
//   // rotor inertia if the rotor is oriented so it spins around the z-axis
//   Mat3<T> rotorRotationalInertiaZ;
//   rotorRotationalInertiaZ << 33, 0, 0, 0, 33, 0, 0, 0, 63; // not changed
//   rotorRotationalInertiaZ = 1e-6 * rotorRotationalInertiaZ; // not changed

//   Mat3<T> RY = coordinateRotation<T>(CoordinateAxis::Y, M_PI / 2); // 绕y转pi/2
//   Mat3<T> RX = coordinateRotation<T>(CoordinateAxis::X, M_PI / 2); // 绕x转pi/2
//   Mat3<T> rotorRotationalInertiaX =
//       RY * rotorRotationalInertiaZ * RY.transpose(); //把电机转子的惯量转到y方向
//   Mat3<T> rotorRotationalInertiaY =
//       RX * rotorRotationalInertiaZ * RX.transpose(); //把电机转子的惯量转到x方向
...
//   Vec3<T> rotorCOM(0, 0, 0); //定义转子的质心位置和空间惯性
//   SpatialInertia<T> rotorInertiaX(0.055, rotorCOM, rotorRotationalInertiaX);
//   SpatialInertia<T> rotorInertiaY(0.055, rotorCOM, rotorRotationalInertiaY);

//   Mat3<T> bodyRotationalInertia; //定义机器人身体的旋转惯性矩和空间惯性
//   bodyRotationalInertia << 11253, 0, 0, 0, 36203, 0, 0, 0, 42673;
//   bodyRotationalInertia = bodyRotationalInertia * 1e-6;
//   Vec3<T> bodyCOM(0, 0, 0);
//   SpatialInertia<T> bodyInertia(cheetah._bodyMass, bodyCOM,
