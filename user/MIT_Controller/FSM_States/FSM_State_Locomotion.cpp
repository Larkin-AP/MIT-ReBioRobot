...
/**
 * FSM_State_Locomotion 构造函数：接收 ControlFSMData<T>* 作为参数，这是一个持有机器人控制数据的类。
 * 调用了基类 FSM_State<T> 的构造函数，传递了状态名称 "LOCOMOTION"。
 * cMPCOld：使用 ConvexMPCLocomotion（一个用于行走控制的模型预测控制器），分别为不同机器人类型 (MINI_CHEETAH 和 CHEETAH_3) 进行初始化。
 * 安全检查：打开了所有安全检查，但关闭了足部位置命令的检查。
 * 初始化变量：footFeedForwardForces 和 footstepLocations 都初始化为零矩阵。
 * 创建控制器：为 WBC（全身控制器，Whole Body Controller）创建数据和控制实例。
 */
/**
 ...
 */
/**
 * LocomotionControlStep：执行行走控制的具体步骤，备份每条腿的控制数据，并根据使用的控制器（如 WBC）运行运动控制逻辑。
 */
template <typename T>
