...
// ---------- new added ---------//
/*
解释：直接通过iter计时，从掉电状态K_PASSIVE切换到位控的站立状态K_RECOVERY_STAND；
切换到力控的站立状态K_BALANCE_STAND；再切换到运动状态K_LOCOMOTION，对应于convexMPC/convexMPCLocomotion.cpp中的控制器，
可以编辑控制器实现预设的运动，包括机身六自由度的运动和不同步态模式的设定；
